...
async def create_post(body: str, async_client: AsyncClient, token: str) -> dict:
...
async def create_comment(body: str, post_id: int, async_client: AsyncClient, token: str) -> dict:
    response = await async_client.post(
        "/comment",
...
async def logged_in_token(async_client: AsyncClient, registered_user: dict):
    return security.create_access_token(registered_user["email"])
...
async def test_get_all_posts(async_client: AsyncClient, created_post: dict, logged_in_token: str):
    response = await async_client.get("/post", headers={"Authorization": f"Bearer {logged_in_token}"})
    assert response.status_code == 200
...
@pytest.mark.anyio
async def test_get_comments_on_post(async_client: AsyncClient, created_post: dict, created_comment: dict, logged_in_token: str):
...
@pytest.mark.anyio
async def test_get_post_with_comments(async_client: AsyncClient, created_post: dict, created_comment: dict, logged_in_token: str):
    response = await async_client.get(
...
@pytest.mark.anyio
async def test_get_missing_post_with_comments(async_client: AsyncClient, logged_in_token: str):
    response = await async_client.get(
        "/post/2",
        headers={"Authorization": f"Bearer {logged_in_token}"},
    )
