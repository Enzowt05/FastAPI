...
async def create_comment(
    body: str, post_id: int, async_client: AsyncClient, token: str
) -> dict:
    response = await async_client.post(
        "/comment",
...
async def like_post(post_id: int, async_client: AsyncClient, token: str) -> dict:
    response = await async_client.post(
        "/like", json={"post_id": post_id}, headers={"Authorization": f"Bearer {token}"}
    )
    return response.json()
...
@pytest.mark.anyio
async def get_all_posts_wrong_sorting(async_cliente: AsyncClient):
    response = await async_cliente.get("/post", params={"sorting": "wrong"})
    assert response.status_code == 422
